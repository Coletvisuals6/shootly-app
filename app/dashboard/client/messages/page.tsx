...
interface Conv { id: string; creator_id: string; creator_name: string; last_message: string; unread: number }
...
export default function ClientMessages() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setUserId(user.id)
      const { data } = await supabase.from('conversations').select('*, creators!conversations_creator_id_fkey(*, profiles(*)), messages(*)').eq('client_id', user.id)
      const formatted: Conv[] = (data || []).map((c: any) => ({
        id: c.id, creator_id: c.creator_id,
        creator_name: c.creators?.profiles?.full_name || 'Creator',
  ...
  useEffect(() => {
    ...
    load()
    const channel = supabase.channel(`client-msgs-${selected}`).on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `conversation_id=eq.${selected}` }, payload => {
    ...
    <div>
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '48px 24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          ...
          <button onClick={() => router.push('/dashboard/client/bookings')} style={{ background: 'transparent', border: '1px solid #2a2a2a', color: '#9ca3af', borderRadius: 8, padding: '8px 14px', fontSize: 13, cursor: 'pointer' }}>← My Bookings</button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 20, height: 560 }}>
          <div style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <div style={{ padding: '14px 16px', borderBottom: '1px solid #2a2a2a' }}><p style={{ fontWeight: 600, fontSize: 14, color: '#9ca3af' }}>Conversations</p></div>
            <div style={{ overflowY: 'auto', flex: 1 }}>
              {convs.length === 0 ? <p style={{ padding: 16, color: '#9ca3af', fontSize: 13 }}>No messages yet</p> : convs.map(c => (
                <div key={c.id} onClick={() => setSelected(c.id)} style={{ padding: '14px 16px', borderBottom: '1px solid #1f1f1f', cursor: 'pointer', background: selected === c.id ? '#1f1535' : 'transparent', borderLeft: `3px solid ${selected === c.id ? '#7c3aed' : 'transparent'}` }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    <p style={{ fontWeight: 600, fontSize: 14 }}>{c.creator_name}</p>
            ...
            {!selected ? (
              ...
              <>
                <div style={{ padding: '14px 20px', borderBottom: '1px solid #2a2a2a' }}>
                  <p style={{ fontWeight: 600 }}>{convs.find(c => c.id === selected)?.creator_name}</p>
                </div>
